    ...
    <table class="table table-hover text-center">
        ...
        <c:forEach items="${pagers.datas}" var="data" varStatus="l">
            <tr>
                ...
                <td style="color: red">
                    <c:if test="${data.status == 0}">待核验</c:if>
                    <c:if test="${data.status == 1}">已取消</c:if>
                    <c:if test="${data.status == 2}">待收货</c:if>
                    <c:if test="${data.status == 3}">已收货</c:if>
                </td>
                <td>
                    <a class="button border-main" href="${ctx}/orderDetail/ulist?orderId=${data.id}"><span
                            class="icon-edit">查看购买餐品</span> </a>
                    <c:if test="${data.status == 0}">
                        <a class="button border-red" href="${ctx}/itemOrder/fh?id=${data.id}"><span
                                class="icon-trash-o">去核验</span> </a>
                    </c:if>
                </td>
        ...
        <tr>
            <td colspan="8">
                <div class="pagelist">
                    <!--分页开始-->
                    <pg:pager url="${ctx}/itemOrder/findBySql" maxIndexPages="5" items="${pagers.total}"
